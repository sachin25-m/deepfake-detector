    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', minHeight: 'calc(100vh - 140px)', justifyContent: 'center', padding: '2rem 0' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', width: '100%', maxWidth: '1100px' }}>
        <FeatureCard 
          ...
          desc="Frame-by-frame temporal consistency checks using multi-frame Vision Transformer inference to catch subtle deepfakes."
          delay="100ms"
        />
        <FeatureCard 
          icon={<ImageIcon size={28} color="var(--secondary)" />} 
          title="Image Forensics" 
          desc="Vision Transformer self-attention detects facial blending seams, boundary warping, and synthetic pixel artifacts."
          delay="200ms"
        />
        <FeatureCard 
          icon={<AlertTriangle size={28} color="var(--accent)" />} 
          title="Risk Mitigation" 
          desc="Protect against synthetic identity manipulation and digital fraud with calibrated ML confidence ratings."
          delay="300ms"
        />
      </div>
    </div>
